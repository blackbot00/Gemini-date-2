...
async def start_human_search(callback: types.CallbackQuery, state: FSMContext):
    ...
    # --- PREMIUM LOGIC: UNLIMITED SEARCH ---
    if not is_premium:
...
async def relay_handler(message: types.Message, state: FSMContext):
    ...
    # --- PREMIUM LOGIC: LINKS & USERNAMES ---
    if message.text:
        forbidden = ["http", ".com", ".in", "@", "t.me", ".net"]
        if not is_premium and any(x in message.text.lower() for x in forbidden):
            return await message.answer("⚠️ Links and Usernames are blocked for Free users! Upgrade to Premium to share. 💎")

    # --- PREMIUM LOGIC: INSTANT MEDIA ---
    if not message.text:
        if not is_premium:
...
async def submit_report(callback: types.CallbackQuery):
    ...
    await callback.bot.send_message(LOG_GROUP_1, f"🚷 **New Report**\nReporter: {callback.from_user.id}\nTarget: {target_id}\nReason: {reason}")
    await callback.message.edit_text("✅ Report submitted to admins.")
